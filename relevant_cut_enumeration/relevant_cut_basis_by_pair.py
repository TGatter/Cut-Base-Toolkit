...
def add_W(G, W1, W2, a, b):

    descW1 = set(nx.descendants(W1, a))
    descW1.add(a)
    ancW1 = set(nx.ancestors(W1, b))
    ancW1.add(b)

    descW2 = set(nx.descendants(W2, a))
    descW2.add(a)
    ancW2 = set(nx.ancestors(W2, b))
    ancW2.add(b)

    combW = nx.DiGraph()
    combW.add_nodes_from(G.nodes())
    for u,v in G.edges():

        # test if uv exists in both w1(a,b) and w2(a,b)
        inW1 = W1.has_edge(u,v) or (u in descW1 and v in descW1) or (u in ancW1 and v in ancW1)
        inW2 = W2.has_edge(u,v) or (u in descW2 and v in descW2) or (u in ancW2 and v in ancW2)

        if inW1 and inW2:
            combW.add_edge(u,v)

        # as we switch undirected to directed, check both directions
        inW1r = W1.has_edge(v,u) or (u in descW1 and v in descW1) or (u in ancW1 and v in ancW1)
        inW2r = W2.has_edge(v,u) or (u in descW2 and v in descW2) or (u in ancW2 and v in ancW2)
...
def add_contract_edges(W, a, b):
    
    desc = list(nx.descendants(W, a))
    desc.append(a)
    for d1 in desc:
        for d2 in desc:
            W.add_edge(d1, d2)

    anc = list(nx.ancestors(W, b))
    anc.append(b)
    for a1 in anc:
        for a2 in anc:
            W.add_edge(a1, a2)
...
def add_W_Matrix(G, W1, W2, a, b):

    W1c = W1.copy()
    W2c = W2.copy()
    
    add_contract_edges(W1c, a, b)
    add_contract_edges(W2c, a, b)

    M1 = nx.to_numpy_array(W1c,  nodelist=sorted(G.nodes()))
    M2 = nx.to_numpy_array(W2c,  nodelist=sorted(G.nodes()))
              
    combM = np.logical_and(M1.astype(bool), M2.astype(bool))
    combM = combM.astype(int)

    combW = nx.DiGraph()
    nodelabels = list(sorted(G.nodes()))

    for i in range(len(combM)):
        for j in range(len(combM)):
            if combM[i, j] == 1 and G.has_edge(i,j):
                combW.add_edge(nodelabels[i], nodelabels[j])  # Use node labels instead of indices

    return combW


def recursive_tree_enumeration_for_closure_graphs(G, T, pi, i, mincut, a, W, all_st_dags):
    #b is always the next j    

    for j in T.neighbors(i):
        if j == pi:
            continue

        if T[i][j]["weight"] > mincut:
            recursive_tree_enumeration_for_closure_graphs(G, T, i, j, mincut, a, W, all_st_dags)
        elif T[i][j]["weight"] < mincut:
            recursive_tree_enumeration_for_closure_graphs(G, T, i, j, T[i][j]["weight"], a, all_st_dags[(i,j)][0], all_st_dags)
        else:  # T[i][j]["weight"] == mincut:
            comb_W = add_W(G, W, all_st_dags[(i,j)][0], a, j)
            #comb_W = add_W_Matrix(G, W, all_st_dags[(i,j)][0], a, j)

            recursive_tree_enumeration_for_closure_graphs(G, T, i, j, mincut, a, comb_W, all_st_dags)
            all_st_dags[(a,j)] = (comb_W, mincut)
...
def enumerate_all_minimum_cuts(G):
    ...
    # store all DAGs created, so we can later enumerate the min-cuts from them
    all_st_dags = {}
    
    # compute W for all edges so we avoid recomputation
    for u,v in T.edges():
        tcg = fast_reduced_transitive_closure_graph(G,u,v)
        all_st_dags[ (u,v) ] = (tcg, T[u][v]["weight"])
        #condensed_print_graph(tcg, "DAG_"+str(u)+"_"+str(v))
        
        tcgrev = fast_reduced_transitive_closure_graph(G,v,u)
        all_st_dags[ (v,u) ] = (tcgrev, T[u][v]["weight"])
        #condensed_print_graph(tcgrev, "DAG_"+str(v)+"_"+str(u))

    # find all DAGs for paths starting at each node i
    for i in T.nodes():
        for j in T.neighbors(i):
            recursive_tree_enumeration_for_closure_graphs(G, T, i, j, T[i][j]["weight"], i, all_st_dags[(i,j)][0], all_st_dags)
    dag_by_size = {}
    for (u,v), (W, weight) in all_st_dags.items():
        ...
        dag_by_size.setdefault(weight, []).append(dag)
    ...
    cut_base = []
    cut_weights = []
    for weight in sorted(dag_by_size):

        for dag in dag_by_size[weight]:      
    ...
    #for l in sorted([str(x) for x in full_cut_list]):
    #    print(l)
